import Foundation
import Security

/// Secure storage using iOS Keychain. Data persists across app reinstalls.
enum KeychainService {

    private static let service = "com.irl.app"

    // MARK: - Keys

    static let authTokenKey = "irl_auth_token"
    static let userIdKey = "irl_user_id"
    static let biometricKeyIdKey = "irl_biometric_key_id"

    // MARK: - Save

    static func save(key: String, value: String) {
        guard let data = value.data(using: .utf8) else { return }

        // Delete existing first
        delete(key: key)

        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: service,
            kSecAttrAccount as String: key,
            kSecValueData as String: data,
            kSecAttrAccessible as String: kSecAttrAccessibleAfterFirstUnlock,
        ]

        SecItemAdd(query as CFDictionary, nil)
    }

    // MARK: - Load

    static func load(key: String) -> String? {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: service,
            kSecAttrAccount as String: key,
            kSecReturnData as String: true,
            kSecMatchLimit as String: kSecMatchLimitOne,
        ]

        var result: AnyObject?
        let status = SecItemCopyMatching(query as CFDictionary, &result)

        guard status == errSecSuccess,
              let data = result as? Data,
              let string = String(data: data, encoding: .utf8) else {
            return nil
        }

        return string
    }

    // MARK: - Delete

    static func delete(key: String) {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: service,
            kSecAttrAccount as String: key,
        ]

        SecItemDelete(query as CFDictionary)
    }

    // MARK: - Clear all

    static func clearAll() {
        delete(key: authTokenKey)
        delete(key: userIdKey)
        delete(key: biometricKeyIdKey)
    }

    // MARK: - Helpers

    static var isLoggedIn: Bool {
        load(key: authTokenKey) != nil && load(key: userIdKey) != nil
    }

    static var userId: String? {
        load(key: userIdKey)
    }

    static var authToken: String? {
        load(key: authTokenKey)
    }
}
